...
- City/domain-shift reporting from exported metadata, kept separate from random holdout numbers, with a worst-slice macro F1 release gate.
- Weak-slice watchlist rows for obstacle and surface-problem heads so data collection targets the domains most likely to hurt accessibility usefulness.
...
- Keep city/domain-shift performance separate from random split performance. A strong random split does not prove cross-city generalization.
- Treat the worst city/domain slice as a production gate, not only a dashboard number.
...
- Worst city/domain macro F1 passed the current release floor: `0.5092` against `>= 0.50`.
- Serving latency passed with zero failed requests: p95 `409.453 ms` on the real RampNet attempt and p95 `105.289 ms` on the smoke run.
