...
| Route graph load | `RouteGraphRepository.LoadGraphAsync` | `route_edges` bbox scan on imported OSM graph can grow quickly. | `IX_route_edges_geometry_gist` supports `ST_Intersects`; runtime caps `Routing__MaxRouteGraphEdges`, caches packed prepartitioned shards, and can use `READONLY_DATABASE_URL` for hot read traffic. |
